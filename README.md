...
Relational Database is a place to persistently store the data in a tabular format where each entry has a primary key and all fileds directly relate to each other.
Structures Query Language is a tool to manipulate the data, may use a raw sql, but preferably to wrap the sql in knex file if using js.
